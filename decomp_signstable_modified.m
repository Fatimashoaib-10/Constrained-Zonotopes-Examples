...
%clear; close all; clc;
function [Fmax,Fmin,fupp,floww,H]=decomp_signstable_modified(f,xup,xlow,dfup,dflow)
%clear; close all; clc;
n=size(xlow,1);m=size(dfup,1);%lambda=ppermute(n);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Bpp=zeros(size(dfup));Bp=zeros(size(dfup));
fupp=zeros(2^(m*n),m);floww=zeros(2^(m*n),m);
H=zeros(2^(m*n),n);
%% sign-stability conditios
for j=1:n
    for i=1:m
        Bpp(i,j)=min(-eps-dfup(i,j),0);%negative sign-stable
        Bp(i,j)=max(eps-dflow(i,j),0);%positive sign-stable
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for s=1:2^(m*n)
   ...
   fup=zeros(size(Fmax));flow=zeros(size(Fmin));
    ...
    for i=1:m
        ...
        fupp(s,i)=fcup(i)+B(i,:)*xcup(i,:)';
        floww(s,i)=fclow(i)+B(i,:)*xclow(i,:)';
        H(s,:)=-B;
    end
    Fmax=min(Fmax,fup);
    Fmin=max(Fmin,flow);
